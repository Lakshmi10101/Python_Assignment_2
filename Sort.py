...
class MergeSort(Sort):
    ...
    def _merge_sort(self, items):
        ...
        mid_index = N//2                        # Finding the mid of the list
        # Splitting the list into two halves
        left_half = items[:mid_index]
        right_half = items[mid_index:]
        
        if len(left_half) > 1: 
            left_half = self._merge_sort(left_half)        # Recursively sorting the first half using MergeSort
        if len(right_half) > 1:
            right_half = self._merge_sort(right_half)      # Recursively sorting the second half using MergeSort
        
        sorted_items = self._merge(left_half, right_half)  # Merging the sorted haves in order
